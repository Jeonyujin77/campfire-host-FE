...
import { __modifyCampAmenities, __modifyCampEnvs, __modifyCampThemes, __modifyCampTypes } from "../../apis/campApi";


const KeywordsList = ({ keywordList, campId }: { keywordList: KeywordsInfo, campId: string }) => {
    ...
    const onModifyCampAmenities = () => {
        dispatch(__modifyCampAmenities({ campId, keywordList: amenities })).then((res) => {
    ...
    const onModifyCampThemes = () => {
        dispatch(__modifyCampThemes({ campId, keywordList: themes })).then((res) => {
            const { type, payload } = res;
            // 등록 성공
